...
class CouponDownloadServiceTest {
    ...
    class SuccessCase {
        ...
        void downloadWithValidCode() {
            // given
            command = DownloadCouponCommand.of("SUMMER2024", 100L);
            ...
            verify(saveCouponPolicyPort).decrementStock(codePolicy.getId());
        ...
        void downloadWithinUserLimit() {
            ...
            // then
            assertThat(result).isNotNull();
            verify(loadCouponIssuePort).countUserIssuance(100L, codePolicy.getId());
        }
    }
    ...
    class FailureCase {
        ...
        void downloadWithInvalidCode() {
            // given
            command = DownloadCouponCommand.of("INVALID_CODE", 100L);
        ...
        void downloadExpiredCoupon() {
            // given
            command = DownloadCouponCommand.of("EXPIRED2024", 100L);
            // loadByCodeAndActive는 활성화된 쿠폰만 반환하므로 만료된 쿠폰은 조회되지 않음
        ...
        void downloadSoldOutCoupon() {
            ...
            command = DownloadCouponCommand.of("SOLDOUT2024", 100L);
            when(loadCouponPolicyPort.loadByCodeAndActive("SOLDOUT2024"))
                    .thenReturn(Optional.of(soldOutPolicy));
            when(loadCouponIssuePort.countUserIssuance(100L, soldOutPolicy.getId()))
                    .thenReturn(0);
            when(saveCouponPolicyPort.decrementStock(soldOutPolicy.getId()))
        ...
        void downloadInactiveCoupon() {
            // given
            command = DownloadCouponCommand.of("INACTIVE", 100L);
    ...
    class ConcurrencyTest {
        ...
        void concurrentDownload() throws InterruptedException {
            ...
            when(loadCouponIssuePort.countUserIssuance(anyLong(), eq(limitedPolicy.getId())))
                    .thenReturn(0);
            when(saveCouponPolicyPort.decrementStock(limitedPolicy.getId()))
            ...
            for (int i = 0; i < threadCount; i++) {
                ...
                executor.submit(() -> {
                    try {
                        ...
                        DownloadCouponCommand cmd = DownloadCouponCommand.of("LIMITED", userId);
    ...
    class EdgeCase {
        ...
        void emptyOrBlankCouponCode(String couponCode) {
            // when & then
            assertThatThrownBy(() -> DownloadCouponCommand.of(couponCode, 100L))
                    .isInstanceOf(IllegalArgumentException.class)
                    .hasMessage("쿠폰 코드는 필수입니다");
        }
        ...
        void nullCouponCode() {
            // given & when & then
            assertThatThrownBy(() -> DownloadCouponCommand.of(null, 100L))
                    .isInstanceOf(IllegalArgumentException.class)
                    .hasMessage("쿠폰 코드는 필수입니다");
        }
        ...
        void invalidUserId() {
            // given & when & then
            assertThatThrownBy(() -> DownloadCouponCommand.of("SUMMER2024", null))
                    .isInstanceOf(IllegalArgumentException.class)
                    .hasMessage("유효한 사용자 ID가 필요합니다");

            assertThatThrownBy(() -> DownloadCouponCommand.of("SUMMER2024", -1L))
                    .isInstanceOf(IllegalArgumentException.class)
                    .hasMessage("유효한 사용자 ID가 필요합니다");
        }
    ...
    class PerformanceTest {
        ...
        void sequentialDownloadPerformance() {
            ...
            for (int i = 0; i < userCount; i++) {
                DownloadCouponCommand cmd = DownloadCouponCommand.of("UNLIMITED", (long) (100 + i));
                CouponIssue result = couponDownloadService.downloadCoupon(cmd);
                assertThat(result).isNotNull();
            }
